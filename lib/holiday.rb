...
def add_supply_to_memorial_day(holiday_hash, supply)
  ...
  holiday_hash[:spring].each {|k, v| v.push(supply)}
end
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_supplies.each do |k, v|
    ...
    v.each do |x, y|
      holiday = x.to_s.capitalize
      combine = "  " + "#{holiday}: #{y.to_s}".tr('[_]', ' ').tr('""', '').split.map(&:capitalize).join(' ')
      if combine.include?("Bbq")
        puts combine.tr('Bbq', "BBQ")
      else
        puts combine
      end
    end
